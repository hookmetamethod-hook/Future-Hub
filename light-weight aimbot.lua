...
--// Requirement checks
if getfenv().readfile and getfenv().writefile and getfenv().isfile then --[[nothing]] if _G.FHdebug then print("readfile, writefile and isfile are available") end else; if _G.FHdebug then print("readfile, writefile or isfile are not available, warning user of issue") end; Notify("An error occured when checking read/write functions, read dev console for more info.", "Future Hub | Adonis Speed Bypass"); _G.FH_Status_AdonisSpeedBypass = nil; error("Future Hub | Adonis Speed Bypass | Your exploit does not have readfile, writefile or isfile, those are required to use this script.", 2); if _G.FHdebug then print("warned user of issue") end end
--// FutureHub env check
